...
const ViewHeader = (props: Props) => {
    ...
    const [pendingViewType, setPendingViewType] = useState<IViewType | null>(
        null,
    )
    const intl = useIntl()
    ...
    const showView = useCallback(
        (viewId: string) => {
            let newPath = generatePath(Utils.getBoardPagePath(match.path), {
                ...match.params,
                viewId: viewId || "",
            })
            if (props.readonly) {
    ...
    const createViewWithVisibility = useCallback(
        (viewType: IViewType, visibility: ViewVisibility) => {
            Utils.log(`addview-${viewType}`)
            TelemetryClient.trackEvent(
                TelemetryCategory,
                TelemetryActions.CreateBoardView,
                { board: board.id, view: activeView.id },
            )
            const view = createBoardView()

            switch (viewType) {
            case "board":
                view.title = intl.formatMessage({
                    id: "View.NewBoardTitle",
                    defaultMessage: "Board view",
                })
                break
            case "table":
                view.title = intl.formatMessage({
                    id: "View.NewTableTitle",
                    defaultMessage: "Table view",
                })
                view.fields.visiblePropertyIds = board.cardProperties.map(
                    (o: IPropertyTemplate) => o.id,
                )
                view.fields.columnWidths = {}
                view.fields.columnWidths[Constants.titleColumnId] =
                        Constants.defaultTitleColumnWidth
                break
            case "gallery":
                view.title = intl.formatMessage({
                    id: "View.NewGalleryTitle",
            ...
            case "calendar":
                view.title = intl.formatMessage({
                    id: "View.NewCalendarTitle",
                    defaultMessage: "Calendar view",
                })
                view.parentId = board.id
                view.fields.visiblePropertyIds = [Constants.titleColumnId]
                view.fields.dateDisplayPropertyId =
                        board.cardProperties.find(
            ...
            view.fields.visibility =
                visibility === "everyone" ? "everyone" : "owner-only"
